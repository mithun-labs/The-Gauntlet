...
- **Retrieved:** 2026-06-22 18:33 UTC (last PRIMARY fetch) — **refresh attempt 2026-06-29 FAILED (primary unreachable, see below); carried forward.**
- **Rulebook version (agent-assigned):** v1
- **Change note:** initial. **2026-06-22 freshness refresh:** forbidden-strategies article re-fetched from the primary Intercom help center — prohibited-strategy list and EA policy ("third-party EA allowed only as a trade/risk manager") UNCHANGED. No version bump. Numeric model parameters still secondary/UNCONFIRMED. **2026-06-29 freshness refresh FAILED — primary now UNREACHABLE:** Funding Pips has **migrated its help center from Intercom to Zendesk** (`help.fundingpips.com/hc/en-us/articles/34505029138449-Trading-Conduct-and-Security-Standards`), which returns **HTTP 403** (bot protection) to the fetch tool; the previously-working Intercom workspace path (`intercom.help/fundingpips/.../8536000-...`) now returns **HTTP 404** (migrated away). No Funding Pips primary page could be fetched this run. A WebSearch AI summary (lead only, NOT a fetched page per HARD RULE 2) is consistent with the stored policy (third-party EA permitted **only** as a trade/risk manager; prohibited: gap trading, HFT, server spamming, latency arbitrage, toxic flow, hedging, long-short/reverse arbitrage, tick scalping, server-execution exploits, opposite-account trading, third-party copy/management). **No rule change made from the summary.** EA-policy + banned-list remain the 2026-06-22 PRIMARY-CONFIRMED text; treat as policy-stable but flag the access change for the operator.
