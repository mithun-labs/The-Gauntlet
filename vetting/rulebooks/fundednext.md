...
- **Retrieved:** 2026-06-19 03:53 UTC — **STALE — refresh failed 2026-06-29 (HTTP 503 again on both help-center articles `8020763` and `8020351`, 7th consecutive run).**
...
- **2026-06-29 refresh attempt FAILED:** the two help-center articles surfaced by search (`help.fundednext.com/en/articles/8020763-is-ea-allowed-in-fundednext` and `.../8020351-what-are-the-restricted-prohibited-trading-strategies`) again returned **HTTP 503** to the fetch tool — 7th consecutive failed run. A WebSearch AI summary (lead only, NOT a fetched primary page per HARD RULE 2) remains consistent with the stored rulebook (EAs allowed on MT4/MT5 with a fee + customization; HFT/latency/order-flooding prohibited; strategy-consistency clause forbids switching manual↔EA between challenge and funded). **No rule change made from the summary.**
- **STATUS: PROVISIONAL / UNCONFIRMED / STALE.** Every rule below is from a secondary aggregator, not FundedNext's primary site. Per HARD RULE 11 and FIRM RULEBOOKS, no Deployable verdict may rest on these rules, and per-firm legality judged against this rulebook is at best `Conditional`. **ESCALATED TO OPERATOR: FundedNext primary site (help.fundednext.com) has been unreachable (HTTP 503) across 7 consecutive runs — the agent cannot verify FundedNext's rules from a primary source. Operator should decide whether an unverifiable-rulebook firm should continue to gate verdicts, or supply an alternate primary source.**
